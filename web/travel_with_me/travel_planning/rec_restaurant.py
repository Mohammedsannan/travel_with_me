...
def recommendor(id):
    stuentid=id
    q="select * from travel_planning_restaurant"
    rescrs=selectall(q)
    ...
    for i in resusers:
        row=[]
        crsrow=[]
        qry="SELECT `restaurantid_id` FROM `travel_planning_restaurantreviewrating` WHERE `uid_id`="+str(i[6])
        ...
        for ii in crslist:
            if ii in crsrow:
                ...
                row.append(0)
        dataset.append(row)
    qry = "SELECT `restaurantid_id` FROM `travel_planning_restaurantreviewrating` WHERE `uid_id`=" + str(id)
    crssele = selectall(qry)
    crsrow=[]
    row=[]
    for ii in crssele:
